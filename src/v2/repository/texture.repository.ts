...
export async function search(
	...
	if (!nameOrId) {
		...
		return Object.values(await getRaw());
	}
...
export async function searchProperty<Property extends TextureProperty>(
	...
	const results = await search(nameOrID, tag);
...
export async function getURLById(id: number, pack: PackID, version: string): Promise<string> {
	const tex = await textures.get(id);
	return tex.url(pack, version);
}

export async function getEditions(): Promise<string[]> {
	const res = await uses.values({ field: "edition" });
	return res.sort();
}

export async function getResolutions(): Promise<number[]> {
	const res = await packs.values({ field: "resolution" });
	return res.sort();
}

export async function getAnimated(): Promise<number[]> {
...
export async function getVersionByEdition(edition: Edition): Promise<string[]> {
...
export async function createTexture(texture: TextureCreationParam): Promise<Texture> {
	const id = await textures.add(texture);
	return textures.get(id);
}

export async function createTexturesBulk(textureArr: TextureCreationParam[]): Promise<Texture[]> {
	const ids = await textures.addBulk(textureArr);
	return textures.searchKeys(ids);
}

export async function editTexture(id: string, body: TextureCreationParam): Promise<Texture> {
	const unmapped = { id, ...body };
	await textures.set(id, unmapped);
	return textures.get(id);
}

export async function deleteTexture(id: string): Promise<WriteConfirmation[]> {
