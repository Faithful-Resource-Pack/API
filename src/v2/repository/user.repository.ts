...
export async function changeUserID(oldID: string, newID: string): Promise<WriteConfirmation> {
	...
	await contributions.editFieldBulk(
		Object.values(rawContributions)
			...
			.map((c) => ({
				...
				value: Array.from(new Set(c.authors.map((author) => (author === oldID ? newID : author)))),
			})),
	);
	...
	await addons.editFieldBulk(
		Object.values(rawAddons)
			...
			.map((a) => ({
				...
				operation: "set",
				// replace old user with new user and remove duplicates
				value: Array.from(new Set(a.authors.map((author) => (author === oldID ? newID : author)))),
			})),
	);
...
export async function remove(id: string): Promise<WriteConfirmation[]> {
	const rawAddons = await addons.readRaw();
...
export async function getUserProfiles(searchedUsers: string[]): Promise<UserProfile[]> {
